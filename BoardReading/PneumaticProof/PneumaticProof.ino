...
void read_all_at_freq(int freq, double * rms_array, double * mag_array, double * phase_array, uint8_t num_elec) // added by david 11/12/23
{
    ...
    // Read from pneumatic chip
    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[2], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[0], MUX_EN);
    mux_write(CHIP_SEL_MUX_VP, elec_to_mux[3], MUX_EN);
...
/* // dsh
   ...
   delay(100);
    ...
    calibrate_samples();
    AD5270_Set(CHIP_SEL_MEAS, 400); // MANUALLY SET GAINS
    AD5270_Set(CHIP_SEL_DRIVE, 400);
...
{   
  ...
  if (millis() - t0 > 350) {
  AD5930_Set_Start_Freq(1000);
  calibrate_samples_at_freq(1000);
  read_all_at_freq(1000, signal_rms, signal_mag, signal_phase, NUM_ELECTRODES);
  for(i = 0; i < 1; i++)
  {       
