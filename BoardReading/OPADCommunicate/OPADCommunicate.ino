...
void read_all_at_freq(int freq, double * rms_array, double * mag_array, double * phase_array, uint8_t num_elec) // added by david 11/12/23
{
  ...
  for(int src_pin = 0; src_pin < num_elec; src_pin++)
  {
    ...
    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[src_pin], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[(sink_pin+4)%num_elec], MUX_EN);

    for(vp_pin = 0; vp_pin < num_elec; vp_pin++)
    {
      if((vp_pin != src_pin)&&(vp_pin != ((src_pin+4)%num_elec))&&(((vp_pin+1)%num_elec) != src_pin)&&(((vp_pin+1)%num_elec) != ((src_pin+4)%num_elec)))
      {

          mux_write(CHIP_SEL_MUX_VP, elec_to_mux[vp_pin], MUX_EN);
          mux_write(CHIP_SEL_MUX_VN, elec_to_mux[(vn_pin+1)%num_elec], MUX_EN);
...
void loop() 
{
  ...
  switch (inByte) {
    case 'y': // Record and return data
      digitalWrite(AD5930_CTRL_PIN, HIGH);
      activate = 1;
      break;
    case 'n': // Stop recording
      digitalWrite(AD5930_CTRL_PIN, LOW);
      activate = 0;
    default:
      break;
  ...
  if (activate) {
    ...
    for(uint16_t i = 0; i < NUM_ELECTRODES*(NUM_ELECTRODES-4); i++)
    {       
