...
void read_all_at_freq(int freq, double * rms_array, double * mag_array, double * phase_array, uint8_t num_elec) // added by david 11/12/23
{
    ...
    // Read from FSR 1
    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[0], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[2], MUX_EN);
    ...
    // Read from FSR 2
    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[3], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[5], MUX_EN);
    mux_write(CHIP_SEL_MUX_VP, elec_to_mux[4], MUX_EN);
    mux_write(CHIP_SEL_MUX_VN, elec_to_mux[6], MUX_EN);
...
/* // dsh
   ...
   delay(100);
    ...
    calibrate_samples();
    AD5270_Set(CHIP_SEL_MEAS, 500); // MANUALLY SET GAINS
    AD5270_Set(CHIP_SEL_DRIVE, 200);
...
{   
  ...
  calibrate_samples_at_freq(TEST_FREQ);
  read_all_at_freq(TEST_FREQ, signal_rms, signal_mag, signal_phase, NUM_ELECTRODES);
  for(i = 0; i < 1; i++)
  {       
